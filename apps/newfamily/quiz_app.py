...
@quiz_app.route('/quiz')
def quiz():
    return render_template('newfamily/quiz/quiz.html')


# 路由，刷新'答题页'
@quiz_app.route('/quiz_question')
def quiz_question():
    return render_template('newfamily/quiz/quiz.html')


# 路由，'答题页'
@quiz_app.route('/quiz_question', methods=['POST'])
def get_question():
    nickname = request.form['nickname']
    return render_template('newfamily/quiz/quiz_question.html',
                           t_nickname=nickname)


# 路由，刷新‘结果页’
@quiz_app.route('/quiz_result')
def get_quiz_result():
    return render_template('newfamily/quiz/quiz.html')
...
def quiz_result():
    ...
    # 返回结果页面
    return render_template('newfamily/quiz/quiz_result.html',
                           t_role=role, t_nickname=nickname)


# 路由，’保存装扮‘
@quiz_app.route('/dress_result', methods=['POST'])
def dress_result():
    nickname = request.form['nickname']
    ...
    return render_template('newfamily/quiz/dress.html', t_dress=dress_list)


# 路由，'直接查看装扮大观园‘
@quiz_app.route('/dress_result')
def dress_list():
    dress_list = list(db.dress.find({}))
    return render_template('newfamily/quiz/dress.html', t_dress=dress_list)
